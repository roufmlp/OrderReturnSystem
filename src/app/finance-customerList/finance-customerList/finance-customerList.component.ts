...
@Component({
  selector: 'app-Finance-customerList',
  templateUrl: './Finance-customerList.component.html',
  styleUrls: ['./Finance-customerList.component.css'],
})
export class FinanceCustomerListComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    localStorage.removeItem("finance-orderDetails");
    ...
    // console.log("Begin");
    this._loadingService.start();
    this._customerService.FinanceList(this.UserDetails).subscribe((res: any) => {
      // debugger;
      if (res.length<=0) {
    ...
    this.filteredCustomerDetails=this.CustomerDetails.filter(function(el) {       
      ...
      if(el.UpdatedBy && el.UpdatedBy.toLocaleLowerCase().includes(txtSearch)) {
        ...
        let filteredProducts:Array<ProductDetailsModel>= el.ProductDetails.filter(function (pr) {          
          if((pr.ProductName && pr.ProductName.toLocaleLowerCase().includes(txtSearch)) || (pr.ProductType && pr.ProductType.toLocaleLowerCase().includes(txtSearch)) || (pr.ReasonForReturn && pr.ReasonForReturn.toLocaleLowerCase().includes(txtSearch)) || (pr.WarehouseComment && pr.WarehouseComment.toLocaleLowerCase().includes(txtSearch)) ){            
            return true;
          } else{
            // console.log("pr.WarehouseComment", pr.WarehouseComment);
  ...
  BoxClicked(customer,products){
    ...
    localStorage.setItem("finance-orderDetails","true");
    this._router.navigate(['/finance-orderDetails']);
  }
  LogOut(){
    // console.log("logout")
    this._router.navigate(['/']);
